...
//件名取得
$title=$ms->title_in($_SESSION["e-id"]);
//詳細取得
$message=$ms->main_contents($_GET["id"]);
//詳細取得
$response=$ms->res_contents($_GET["id"]);
//セッション確認
//var_dump($_SESSION);
//データベース切断
$stmt=null;
        ...
        <main>
            ...
            <article>
                <section id="notification">
                    <h1>メッセージ</h1>
                    <ul class="info" style="background-color:<?php echo ($message["in_n"]==$_SESSION["e-id"])? "aliceblue":"pink";?>">
                        <ul>
                            <li>
                                <dl style="display:flex">
                                    <dt><b>送信者：</b></dt>
                                    <dd><?php echo h(staff($message["in_n"]));?></dd>
                                </dl>
                            </li>
                            <li>
                                <dl style="display:flex">
                                    <dt><b>宛先：</b></dt>
                                    <dd><?php echo ($message["out_n"]==0)? "管理者":h(staff($message["out_n"]));?></dd>
                                </dl>
                            </li>  
                        </ul>
                        <li>
...
<?php foreach($response as $value):?>
    <ul class="info" style="background-color:<?php echo ($value["in_n"]==$_SESSION["e-id"])? "aliceblue":"pink";?>">
                        <ul>
                            <li>
                                <dl style="display:flex">
                                    <dt><b>送信者：</b></dt>
                                    <dd><?php echo h(staff($value["in_n"]));?></dd>
                                </dl>
                            </li>
                            <li>
                                <dl style="display:flex">
                                    <dt><b>宛先：</b></dt>
                                    <dd><?php echo ($value["out_n"]==0)? "管理者":h(staff($value["out_n"]));?></dd>
                                </dl>
                            </li>  
                        </ul>
                        <li>
...
<?php endforeach;?>
<?php endif; ?>
<?php if($message["in_n"]!=$_SESSION["e-id"]): ?>
                    <hr>
                    <h2>返信</h2>
...
<?php endif; ?>
                            ...
                            <dl class="m-bottom5px">
                                    <dt>宛先</dt>
                                    <dd><?php echo($message["in_n"]!=$_SESSION["e-id"])? h(staff($message["in_n"])):h(staff($message["out_n"])); ?></dd>
<?php if(isset($output["err-name"])) :?>
                                    <dd class="error"><?php echo h($output["err-name"]); ?></dd>
<?php endif; ?>
                                </dl>
...
<?php endif; ?>
                    ...
                    <input type="hidden" name="out_id" id="out_id" value="<?php echo($message["in_n"]!=$_SESSION["e-id"])? h($message["in_n"]):h($message["out_n"]); ?>">
                    </form>
<?php else: ?>
                    <p style="color:red;">*送信先が本人なので返信できません。</p>
<?php endif; ?>
                    <hr>
                    <h2>受信一覧</h2>
                    <p>クリックすると詳細が見れます。</p>

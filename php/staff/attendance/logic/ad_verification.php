...
*/
    ...
    if(!isset($input["Day"]) || !strlen($input["Day"])){
        ...
        $n_year=$year;
        $n_month=sprintf("%02d",$month);
        $n_day=sprintf("%02d",$day);
        $f_year=$input["Year"];
        $f_month=sprintf("%02d",$input["Month"]);
        $f_day=sprintf("%02d",$input["Day"]);
        $N_TIME=date('Y.m.d',strtotime($n_year.$n_month.$n_day));
        $N_TIME=date('Ymd',strtotime('-2 day'));
        $F_TIME=date('Ymd',strtotime($f_year.$f_month.$f_day));
        if((int)$N_TIME > (int)$F_TIME){
            $output["err-day"]="２日前以前の勤怠の登録は出来ません。";
        }
    }

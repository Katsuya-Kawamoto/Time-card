...
*/
    ...
    function time_calculation($result){
        ...
        foreach($result as $value){
            if($value["work_type"]==1){
                $holiday_time=$value["over_time"];
                $holiday_minutes=$value["over_minutes"];
            }
        ...
        //休日出勤時間
        $holiday=time_ip($holiday_time,$holiday_minutes);
        //時間と分に細分化
        $output["holiday_time"]=$holiday["time"];
        $output["holiday_minutes"]=$holiday["minutes"];

        $output["OVER_time"]=$over["time"]-$holiday["time"];
        $output["OVER_minutes"]=$over["minutes"]-$holiday["minutes"];
    ...
    function time_count($result){
        ...
        $output["holiday_count"]    =$holiday_work_count;                           //休日出勤勤務
        $output["OVER_count"]       =$over_time_count-$holiday_work_count;          //残業
